            ...
            switch (aid) {
                case 'init':
                    ...
                    if (lastEvent != 'NULL') {
                        ...
                        curReels += ('&rs.i0.r.i1.pos=' + reels.rp[0]); // Note: PHP uses rp[0] for all positions in this block, likely a copy-paste error in PHP source but we replicate
                ...
                case 'spin':
                    ...
                    const winTypeTmp = slotSettings.GetSpinSettings(postData['slotEvent'], betline * lines, lines);
                    ...
                    for (let i = 0; i <= 2000; i++) {
                        ...
                        let tmpReels = JSON.parse(JSON.stringify(reels));
                        wildStr = '';
                        wildStrArr = [];
                        ...
                        let wc = 0;
                        for (let r = 0; r < 200; r++) {
                            const rew0 = PhpHelpers.rand(1, 5);
                            const rew = PhpHelpers.rand(0, 2);
                        ...
                        for (let k = 0; k < lines; k++) {
                            ...
                            for (let j = 0; j < slotSettings.SymbolGame!.length; j++) {
                                ...
                                if (csym == scatter || !slotSettings.Paytable['SYM_' + csym]) {
                                    ...
                                    if ((s[0] == csym || wild.indexOf(s[0]) !== -1) && (s[1] == csym || wild.indexOf(s[1]) !== -1) && (s[2] == csym || wild.indexOf(s[2]) !== -1)) {
                                        let mpl = 1;
                                    ...
                                    if ((s[0] == csym || wild.indexOf(s[0]) !== -1) && (s[1] == csym || wild.indexOf(s[1]) !== -1) && (s[2] == csym || wild.indexOf(s[2]) !== -1) && (s[3] == csym || wild.indexOf(s[3]) !== -1)) {
                                        let mpl = 1;
                                    ...
                                    if ((s[0] == csym || wild.indexOf(s[0]) !== -1) && (s[1] == csym || wild.indexOf(s[1]) !== -1) && (s[2] == csym || wild.indexOf(s[2]) !== -1) && (s[3] == csym || wild.indexOf(s[3]) !== -1) && (s[4] == csym || wild.indexOf(s[4]) !== -1)) {
                                        let mpl = 1;
                        ...
                        reels = tmpReels; // Logic uses original reels for scatter check, but floating wilds were on top.
                        // Wait, PHP: reels = tmpReels;
                        // But floating wilds were added to 'reels'.
                        // Ah, the floating wilds were added to `reels` object in the PHP code.
                        // Then `tmpReels` (copy of original) is restored.
                        // BUT: `wildStrArr` was built from the modified `reels`.
                        // AND: `lineWins` were built from the modified `reels`.
                        // So `reels` (modified) is used for wins.
                        // Then `reels = tmpReels` restores the original symbols for response string?
                        // Let's check PHP:
                        // $reels = $slotSettings->GetReelStrips(...);
                        // $tmpReels = $reels;
                        // ... floating wilds added to $reels ...
                        // ... win calculation uses $reels ...
                        // $reels = $tmpReels;
                        // ... scatter check uses $reels (which is now original) ...
                        // ... response string built from $reels (original) ...
                        // So the visual reels sent to client are the ORIGINAL ones (without 1s replaced),
                        // but the 1s are sent as OVERLAYS via `wildStr`.
                        ...
                        if (slotSettings.MaxWin! < (totalWin * slotSettings.CurrentDenom)) {
                            ...
                            if (slotSettings.increaseRTP && winType == 'win' && totalWin < (minWin * betline * lines)) {
                                // continue
                    ...
                    if (postData['slotEvent'] == 'freespin') {
                        ...
                        fs = slotSettings.GetGameData('LightsNETFreeGames');
                        const fsl = slotSettings.GetGameData('LightsNETFreeGames') - slotSettings.GetGameData('LightsNETCurrentFreeGame');
                        ...
                        // The PHP code does weird concatenation here:
                        // $curReels = $freeState . $curReels . '&freespins...';
                        // But $freeState was defined above as a long string.
                        // Actually in PHP:
                        // $freeState = 'previous.rs...';
                        // $curReels = $freeState . $curReels . '&freespins...';
                        // $freeState = '&freespins...'; (redefined for response JSON)

                        // We need to match this behavior.
                        // First, update curReels with the hardcoded 'previous.rs...' block + original curReels + new params.

                        const freeStateHeader = 'previous.rs.i0=freespin&freespins.betlevel=1&g4mode=false&freespins.win.coins=0&playercurrency=%26%23x20AC%3B&historybutton=false&current.rs.i0=freespin&rs.i0.r.i4.hold=false&next.rs=freespin&gamestate.history=basic%2Cfreespin&rs.i0.r.i1.syms=SYM12%2CSYM8%2CSYM8&game.win.cents=0&rs.i0.id=freespin&totalwin.coins=0&credit=' + balanceInCents + '&gamestate.current=freespin&freespins.initial=10&jackpotcurrency=%26%23x20AC%3B&multiplier=1&last.rs=freespin&freespins.denomination=5.000&rs.i0.r.i0.syms=SYM5%2CSYM5%2CSYM10&rs.i0.r.i3.syms=SYM10%2CSYM10%2CSYM7&freespins.win.cents=0&freespins.totalwin.coins=0&freespins.total=10&isJackpotWin=false&gamestate.stack=basic%2Cfreespin&rs.i0.r.i0.pos=245&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8&gamesoundurl=&rs.i0.r.i1.pos=21&game.win.coins=0&playercurrencyiso=' + slotSettings.slotCurrency + '&rs.i0.r.i1.hold=false&freespins.wavecount=1&freespins.multiplier=1&playforfun=false&jackpotcurrencyiso=' + slotSettings.slotCurrency + '&clientaction=freespin&rs.i0.r.i2.hold=false&rs.i0.r.i4.syms=SYM6%2CSYM6%2CSYM6&rs.i0.r.i2.pos=75&totalwin.cents=0&gameover=false&rs.i0.r.i0.hold=false&rs.i0.r.i3.pos=146&freespins.left=9&rs.i0.r.i4.pos=245&nextaction=freespin&wavecount=1&rs.i0.r.i2.syms=SYM11%2CSYM11%2CSYM11&rs.i0.r.i3.hold=false&game.win.amount=0.00&freespins.totalwin.cents=0';

                        const suffix = '&freespins.totalwin.cents=0&nextaction=' + nextaction + '&freespins.left=' + fsl + '&freespins.wavecount=1&next.rs=freespin&current.rs.i0=freespin&freespins.multiplier=1&gamestate.stack=' + stack + '&freespins.totalwin.coins=' + totalWin + '&freespins.total=' + fs + '&freespins.win.cents=' + (totalWin / slotSettings.CurrentDenomination * 100) + '&gamestate.current=' + gamestate + '&freespins.initial=' + fs + '&freespins.win.coins=' + totalWin + '&freespins.betlevel=' + slotSettings.GetGameData('LightsNETBet') + '&totalwin.coins=' + totalWin + '&credit=' + balanceInCents + '&totalwin.cents=' + (totalWin * slotSettings.CurrentDenomination * 100) + '&game.win.amount=' + (totalWin / slotSettings.CurrentDenomination);

                        curReels = freeStateHeader + curReels + suffix;

                        // Redefine freeState for the JSON response body
                        freeState = '&freespins.totalwin.cents=0&nextaction=' + nextaction + '&freespins.left=' + fsl + '&freespins.wavecount=1&next.rs=freespin&current.rs.i0=freespin&freespins.multiplier=1&gamestate.stack=' + stack + '&freespins.totalwin.coins=' + totalWin + '&freespins.total=' + fs + '&freespins.win.cents=' + (totalWin * slotSettings.CurrentDenomination * 100) + '&gamestate.current=' + gamestate + '&freespins.initial=' + fs + '&freespins.win.coins=' + totalWin + '&freespins.betlevel=' + slotSettings.GetGameData('LightsNETBet') + '&totalwin.coins=' + totalWin + '&credit=' + balanceInCents + '&totalwin.cents=' + (totalWin * slotSettings.CurrentDenomination * 100) + '&game.win.amount=' + (totalWin / slotSettings.CurrentDenomination);
                    }

                    const response = '{"responseEvent":"spin","responseType":"' + postData['slotEvent'] + '","serverResponse":{"freeState":"' + freeState + '","slotLines":' + lines + ',"slotBet":' + betline + ',"totalFreeGames":' + slotSettings.GetGameData('LightsNETFreeGames') + ',"currentFreeGames":' + slotSettings.GetGameData('LightsNETCurrentFreeGame') + ',"Balance":' + balanceInCents + ',"afterBalance":' + balanceInCents + ',"bonusWin":' + slotSettings.GetGameData('LightsNETBonusWin') + ',"totalWin":' + totalWin + ',"winLines":[],"Jackpots":' + jsJack + ',"reelsSymbols":' + jsSpin + '}}';
                    slotSettings.SaveLogReport(response, betline * lines, lines, reportWin, postData['slotEvent']);
