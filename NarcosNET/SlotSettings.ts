...
export class SlotSettings {
    ...
    public GetRandomScatterPos(rp: any[], rsym: string): number {
        let rpResult: number[] = [];
            ...
            for (let r = 1; r <= 5; r++) {
                if (reels['reel' + r][p] == '2') {
                    if (p == 0 && r == 1) {
                        ...
                        if (reels['reel' + (r - 1)] && reels['reel' + (r - 1)][p] == '2c') {
                            reels['reel' + r][p] = '2c';
                        }
                        if (reels['reel' + r][p - 1] && reels['reel' + r][p - 1] == '2c') {
                            reels['reel' + r][p] = '2c';
                        }
                    }
                }
    ...
    public GetReelStrips(winType: string, slotEvent: string): any {
        ...
        if (winType != 'bonus') {
            ...
            const randomBonusType = PhpHelpers.rand(1, 2);
            if (randomBonusType == 1) {
                ...
                for (let i = 0; i < reelsId.length; i++) {
                 ...
                 const reelsId = [1, 2, 3, 4, 5];
                 const sCnt = PhpHelpers.rand(3, 5);
                 for (let i = 0; i < reelsId.length; i++) {
                     if (i < sCnt) {
