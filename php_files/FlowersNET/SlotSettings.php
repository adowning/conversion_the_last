...
namespace VanguardLTE\Games\FlowersNET
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $context = null)
        {
            ...
            $user = new \VanguardLTE\Stateless\User($context->user);
            $this->user = $user;
            $this->shop_id = $user->shop_id;
            $gamebank = new \VanguardLTE\Stateless\GameBank($context->bank);
        ...
        if ($this->isStateless) return true;
            if( $this->game && $this->shop && $this->user && (!$this->game->view || $this->shop->is_blocked || $this->user->is_blocked || $this->user->status == \VanguardLTE\Stateless\Enum\UserStatus::BANNED) ) 
            {
            ...
            if( $this->user->status == \VanguardLTE\Stateless\Enum\UserStatus::BANNED ) 
            {
                return false;
            }
            return true;
        ...
        if ($this->isStateless) return 'NULL';
            $history = []; // Legacy history retrieval removed
            $this->lastEvent = 'NULL';
        ...
        if ($this->isStateless) return;
            ...
            \VanguardLTE\Stateless\GameLog::create([
