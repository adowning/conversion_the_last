...
namespace App\Games\MMALegendsNG
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $context = null)
        {
            $this->slotId = $sid; $this->isStateless = (bool)$context;
            $this->playerId = $playerId;
            $user = new \App\Stateless\User($context->user);
            $this->user = $user;
            $this->shop_id = $user->shop_id;
            $gamebank = new \App\Stateless\GameBank($context->bank);
            $game = new \App\Stateless\Game($context->game);
            $this->shop = new \App\Stateless\Shop($context->shop);
            ...
            $this->hideButtons = [];
            $this->jpgs = collect($context->jpgs)->map(function($j){ return new \App\Stateless\JPG($j); });
            $this->Line = [
                1, 
                2, 
                3, 
        ...
        if ($this->isStateless) return true;
            if( $this->game && $this->shop && $this->user && (!$this->game->view || $this->shop->is_blocked || $this->user->is_blocked || $this->user->status == \App\Support\Enum\UserStatus::BANNED) ) 
            {
                \App\Session::where('user_id', $this->user->id)->delete();
            ...
            if( $this->user->status == \App\Support\Enum\UserStatus::BANNED ) 
            {
                return false;
            }
            return true;
        ...
        if ($this->isStateless) return 'NULL';
            $history = \App\GameLog::whereRaw('game_id=? and user_id=? ORDER BY id DESC LIMIT 10', [
                $this->slotDBId, 
                $this->playerId
            ])->get();
            $this->lastEvent = 'NULL';
        ...
        if ($this->isStateless) return;
            ...
            $this->game->refresh();
            $gamebank = \App\GameBank::where(['shop_id' => $game->shop_id])->first();
            if( $gamebank ) 
            {
                list($slotsBank, $bonusBank, $fishBank, $tableBank, $littleBank) = \App\Lib\Banker::get_all_banks($game->shop_id);
            }
            else
            {
        ...
        public function GetRandomScatterPos($rp, $bs)
        {
            $rpResult = [];
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            if( $winType != 'bonus' ) 
            {
                ...
                ] as $index => $reelStrip ) 
                {
                    ...
                    {
                        $prs[$index + 1] = mt_rand(1, count($this->$reelStrip) - 3);
                    }
                }
            }
            else
            {
                ...
                ] as $index => $reelStrip ) 
                {
                    if( is_array($this->$reelStrip) && count($this->$reelStrip) > 0 ) 
                    {
                        $prs[$index + 1] = $this->GetRandomScatterPos($this->$reelStrip, $bonusSymbol[0]);
                        $reelsId[] = $index + 1;
                    }
                }
                $scattersCnt = 5;
                ...
                for( $i = 0; $i < count($reelsId); $i++ ) 
                {
                    if( $i < $scattersCnt ) 
                    {
                        $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $bonusSymbol[0]);
                    }
                    else
                    {
                        $prs[$reelsId[$i]] = rand(1, count($this->{'reelStrip' . $reelsId[$i]}) - 3);
                    }
                }
            }
            $reel = [
