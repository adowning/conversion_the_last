...
export class SlotSettings {
    ...
    constructor(sid: string, playerId: number, context: IContext) {
        ...
        this.reelStrip1 = [4,4,5,5,9,9,11,11,13,13,11,11,9,9,13,13,7,7,9,9,13,13,10,10,12,12,11,11,6,6,9,9,7,7,10,10,13,13,11,11,3,3,5,5,11,11,8,8,9,9,12,12,8,8];
        this.reelStrip2 = [13,1,12,10,10,12,9,9,11,11,4,4,12,12,10,10,5,5,1,6,6,10,10,1,7,7,13,13,1,10,10,6,6,9,9,8,8,12,12,10,10,3,3,11,11,12,12,8,8,13];
        this.reelStrip3 = [3,9,9,12,12,1,4,4,9,9,6,6,13,13,11,11,8,8,6,6,13,13,11,11,9,9,13,13,7,7,1,11,11,4,4,13,13,10,10,1,11,11,9,9,1,7,7,5,5,12,12,1,3];
        this.reelStrip4 = [11,10,10,8,8,12,12,5,5,10,10,8,8,12,12,5,5,10,10,7,7,12,12,10,10,1,4,4,6,6,12,12,7,7,11,11,8,8,12,12,4,4,7,7,13,13,12,12,10,10,3,3,6,6,12,12,10,10,4,4,10,10,9,9,6,6,13,13,11];
        this.reelStrip5 = [5,5,13,13,3,3,5,5,10,10,4,4,9,9,6,6,13,13,10,10,11,11,12,12,11,11,13,13,7,7,1,13,13,11,11,7,7,5,5,12,12,3,3,9,9,6,6,9,9,13,13,6,6,11,11,13,13,8,8,11,11,13,13,7,7,13,13,11,11,9,9,12,12];
        this.reelStrip6 = [];

        this.reelStripBonus1 = [4,4,5,5,9,9,11,11,13,13,11,11,9,9,13,13,7,7,9,9,13,13,10,10,12,12,11,11,6,6,9,9,7,7,10,10,13,13,11,11,3,3,5,5,11,11,8,8,9,9,12,12,8,8];
        this.reelStripBonus2 = [13,1,12,10,10,12,9,9,11,11,4,4,12,12,10,10,5,5,6,6,10,10,7,7,13,13,10,10,6,6,9,9,8,8,12,12,10,10,3,3,11,11,12,12,8,8,13];
        this.reelStripBonus3 = [3,9,9,12,12,1,4,4,9,9,6,6,13,13,11,11,8,8,6,6,13,13,11,11,9,9,13,13,7,7,1,11,11,4,4,13,13,10,10,1,11,11,9,9,7,7,5,5,12,12,1,3];
        this.reelStripBonus4 = [11,10,10,8,8,12,12,5,5,10,10,8,8,12,12,5,5,10,10,7,7,12,12,10,10,1,4,4,6,6,12,12,7,7,11,11,8,8,12,12,4,4,7,7,13,13,12,12,10,10,3,3,6,6,12,12,10,10,4,4,10,10,9,9,6,6,13,13,11];
        this.reelStripBonus5 = [5,5,13,13,3,3,5,5,10,10,4,4,9,9,6,6,13,13,10,10,11,11,12,12,11,11,13,13,7,7,1,13,13,11,11,7,7,5,5,12,12,3,3,9,9,6,6,9,9,13,13,6,6,11,11,13,13,8,8,11,11,13,13,7,7,13,13,11,11,9,9,12,12];
    ...
    public GetSpinSettings(garantType: string = 'bet', bet: any, lines: any) {
        ...
        currentSpinWinChance = linesPercentConfigSpin['line' .concat(curField.toString(), pref)][percentLevel];
        currentBonusWinChance = linesPercentConfigBonus['line' .concat(curField.toString(), pref)][percentLevel];
    ...
    public SymbolUpgrade(reels: any, fCnt: number) {
        let RespinId = this.GetGameData(this.slotId + 'RespinId');
        if (RespinId > 5) {
            RespinId = 5;
        }
        const waysLimit: any[][] = [];
        ...
        const cSym0 = PhpHelpers.rand(4, 13);
        const cSym1 = cSym0 - 1;
        for (let r = 1; r <= 5; r++) {
            const curWays = waysLimit[RespinId][r - 1];
            for (const pw of curWays) {
                 if (reels['reel' + r][pw] == cSym0) {
                     reels['reel' + r][pw] = cSym1;
                     randomwildspArr.push('%28' + (r - 1) + '%2C' + pw + '%29');
                 }
            }
        }
    ...
    public RandomWilds(reels: any, fCnt: number) {
        let RespinId = this.GetGameData(this.slotId + 'RespinId');
        if (RespinId > 5) {
            RespinId = 5;
        }
        ...
        for (let rr = 1; rr <= 50; rr++) {
            for (let r = 2; r <= 5; r++) {
                const curWays = waysLimit[RespinId][r - 1];
                for (const pw of curWays) {
                    if (PhpHelpers.rand(1, 5) == 1 && wcnt < 3) {
                         reels['reel' + r][pw] = 1; // Wild logic
                         randomwildspArr.push('%28' + (r - 1) + '%2C' + pw + '%29');
                         wcnt++;
                    }
                }
