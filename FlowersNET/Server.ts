            ...
            switch(aid) {
                ...
                case 'spin':
                    let linesId: number[][] = [
                        ...
                        [3, 1, 1, 1, 3], [1, 1, 3, 1, 1], [3, 3, 1, 3, 3], [1, 3, 2, 1, 3], [3, 1, 2, 3, 1],
                        [2, 1, 3, 1, 2], [2, 3, 1, 3, 2], [1, 2, 3, 3, 3], [3, 2, 1, 1, 1], [2, 1, 2, 3, 2]
                    ];
                    lines = 30;
                    slotSettings.CurrentDenom = postData['bet_denomination'];
                    slotSettings.CurrentDenomination = postData['bet_denomination'];
                    ...
                    if (postData['slotEvent'] != 'freespin') {
                        ...
                        slotSettings.SetGameData('FlowersNETFreeBalance', (slotSettings.GetBalance() * 100).toFixed(2));
                    } else {
                        ...
                        for (let k = 0; k < lines; k++) {
                            ...
                            for (let j = 0; j < slotSettings.SymbolGame.length; j++) {
                                ...
                                if (csym == scatter || !slotSettings.Paytable['SYM_' + csym]) {
                                    ...
                                    if ((s[0] == parseInt(csym) || wild.includes(String(s[0]))) && (s[1] == parseInt(csym) || wild.includes(String(s[1])))) {
                                         mpl = 1;
                                         scnt = 2;
                                         if (parseInt(csym) >= 3 && parseInt(csym) <= 7) {
                                             let dbsym = parseInt(csym) + 10;
                                         ...
                                         let tmpWin = slotSettings.Paytable['SYM_' + csym][scnt] * betline * mpl * bonusMpl;
                                         if (cWins[k] < tmpWin) {
                                             cWins[k] = tmpWin;
                                             tmpStringWin = '&ws.i' + winLineCount + '.reelset=basic&ws.i' + winLineCount + '.types.i0.coins=' + tmpWin + '&ws.i' + winLineCount + '.pos.i0=0%2C' + (linesId[k][0] - 1) + '&ws.i' + winLineCount + '.pos.i1=1%2C' + (linesId[k][1] - 1) + '&ws.i' + winLineCount + '.types.i0.wintype=coins&ws.i' + winLineCount + '.betline=' + k + '&ws.i' + winLineCount + '.sym=SYM' + csym + '&ws.i' + winLineCount + '.direction=left_to_right&ws.i' + winLineCount + '.types.i0.cents=' + (tmpWin * slotSettings.CurrentDenomination * 100) + '';
                                         }
                                    }
                    ...
                    let response_json = '{"responseEvent":"spin","responseType":"' + postData['slotEvent'] + '","serverResponse":{"freeState":"' + freeState + '","slotLines":' + lines + ',"slotBet":' + betline + ',"totalFreeGames":' + slotSettings.GetGameData('FlowersNETFreeGames') + ',"currentFreeGames":' + slotSettings.GetGameData('FlowersNETCurrentFreeGame') + ',"Balance":' + balanceInCents + ',"afterBalance":' + balanceInCents + ',"bonusWin":' + slotSettings.GetGameData('FlowersNETBonusWin') + ',"totalWin":' + totalWin + ',"winLines":[],"Jackpots":' + jsJack + ',"reelsSymbols":' + jsSpin + '}}';

                    slotSettings.SaveLogReport(response_json, allbet, lines, reportWin, postData['slotEvent']);

                    result_tmp.push("&clientaction=spin&rs.i0.r.i2.hold=false&game.win.cents=" + (totalWin * slotSettings.CurrentDenomination * 100)  + "&rs.i0.r.i2.pos=47&rs.i0.id=basic&totalwin.coins=" + totalWin  + "&credit=" + balanceInCents  + "&totalwin.cents=" + (totalWin * slotSettings.CurrentDenomination * 100)  + "&gamestate.current=basic&gameover=true&rs.i0.r.i0.hold=false&jackpotcurrency=%26%23x20AC%3B&multiplier=1&rs.i0.r.i3.pos=4&rs.i0.r.i4.pos=5&isJackpotWin=false&gamestate.stack=basic&nextaction=spin&rs.i0.r.i0.pos=7&wavecount=1&gamesoundurl=&rs.i0.r.i3.hold=false&game.win.amount=" + (totalWin / slotSettings.CurrentDenomination)  + "'  + curReels + winString);
                    break;
            }
            return result_tmp[0];
        } catch(e) {
