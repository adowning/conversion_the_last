    ...
    public get(request: any, game: any, userId: any, context: IContext) {
        try {
            ...
            switch (aid) {
                ...
                case 'spin':
                    ...
                    if (postData['slotEvent'] != 'freespin') {
                        ...
                        slotSettings.SetGameData('FruitShopChristmasNETFreeBalance', parseFloat(slotSettings.GetBalance().toFixed(2)) * 100);
                        bonusMpl = 1;
                    }
                    else {
                    ...
                    const jackRandom = PhpHelpers.rand(1, 500);
                    let mainSymAnim = '';
                    let totalWin = 0;
                    let lineWins: any[] = [];
                    let reels: any = {};
                    ...
                    for (let i = 0; i <= 2000; i++) {
                        ...
                        for (let k = 0; k < lines; k++) {
                            ...
                            for (let j = 0; j < slotSettings.SymbolGame.length; j++) {
                                ...
                                if (csym == scatter || !slotSettings.Paytable['SYM_' + csym]) {
                                    ...
                                    // 2 symbols check
                                    if ((s[0] == csym || wild.indexOf(s[0]) !== -1) && (s[1] == csym || wild.indexOf(s[1]) !== -1)) {
                                        let mpl = 1;
                                        if (wild.indexOf(s[0]) !== -1 && wild.indexOf(s[1]) !== -1) {
                                            mpl = 1;
                                        }
                                        else if (wild.indexOf(s[0]) !== -1 || wild.indexOf(s[1]) !== -1) {
                                            mpl = slotSettings.slotWildMpl;
                                        }
                                        if (slotSettings.Paytable['SYM_' + csym][2] > 0) {
                                            const tmpWin = slotSettings.Paytable['SYM_' + csym][2] * betline * mpl * bonusMpl;
                                            if (cWins[k] < tmpWin) {
                                                cWins[k] = tmpWin;
                        ...
                        if (scattersCount >= 3) {
                            ...
                            // This part commented out or not used in PHP final string construction?
                            // Checked PHP code: $scattersStr is assigned but not concatenated to anything later?
                            // Wait, $scattersStr is unused in PHP code.
                        }
                        ...
                        if (i > 1500) {
                            const response = '{"responseEvent":"error","responseType":"' + postData['slotEvent'] + '","serverResponse":"Bad Reel Strip"}';
                            return response;
                        }
            ...
            if (context && context.slotId) {
                // slotSettings might be available or not.
                // But typically InternalErrorSilent uses slotSettings.
                // We'll just log.
                console.error(e);
            }
